...
app.get('/get-stock-entry', async (req, res) => {
    const { doctype = 'Stock Entry', fields = '["*"]', page = 1, length = 250, filters = '[]' } = req.query;
...
async function getTotalRecordsforReddys(doctype, filters) {
    
    // This API call fetches just the count of the filtered records
    let url = `https://reddyspharma.accu360.cloud/api/resource/${doctype}?fields=["name"]&filters=${JSON.stringify(filters)}&limit_page_length=*`;
    console.log('Fetching total records URL:', url);
    ...
    try {
        const response = await fetch(url, {
            ...
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `token bd351907a17395a:d2ec72f60c58cb5`
            }
        });
        const data = await response.json();
...
async function getInvoicesForReddys(doctype, fields, start, limit, filters) {
    ...
    try {
        ...
        if (response.ok) {
            // Fetch detailed data in parallel for each record
            console.log(data.data.length)
            const detailedData = await getDataParallelForReddys(doctype, data.data);
            return { success: true, data: detailedData };
        } else {
            return { success: false, error: data };
        }
...
// Fetch detailed data for each invoice in parallel
async function getDataParallelForReddys(doctype, data) {
    const promises = data.map(item => {
        const name = item.name;
        const url = `https://reddyspharma.accu360.cloud/api/resource/${doctype}/${name}`;
...
// Route to get invoices based on pagination
app.get('/get-reddys-invoices', async (req, res) => {
    const { doctype = 'Sales Invoice', fields = '["*"]', page = 1, length = 250, filters = '[]' } = req.query;
...
app.get('/get-reddys-chart-of-accounts', async (req, res) => {
    const { doctype = 'Account', fields = '["*"]', page = 1, length = "*", filters = '[]' } = req.query;
    ...
    // Fetch total number of records matching the filters
    const totalRecords = await getForJournalForReddys(doctype, filters);
    console.log('totalRecords', totalRecords);
    ...
    // Fetch the paginated records
    const listResult = await getJournalOneByOneForReddys(doctype, parsedFields, start, length, filters);
...
app.get('/get-reddys-account-details', async (req, res) => {
    const { doctype = 'Account', name = "", fields = '["*"]', page = 1, length = "*", filters = '[]' } = req.query;
    ...
    // Fetch the paginated records
    const listResult = await detailsForReddys(doctype, name, parsedFields, length, filters);
...
async function detailsForReddys(doctype, name, fields, start, limit, filters) {
    let url = `https://reddyspharma.accu360.cloud/api/resource/${doctype}/${name}`;
...
app.get('/get-reddys-purchase', async (req, res) => {
    const { doctype = 'Purchase Invoice', fields = '["*"]', page = 1, length = 250, filters = '[]' } = req.query;
...
app.get('/get-reddys-payment', async (req, res) => {
    const { doctype = 'Payment Entry', fields = '["*"]', page = 1, length = 250, filters = '[]' } = req.query;
...
app.get('/get-reddys-journal', async (req, res) => {
    const { doctype = 'Journal Entry', fields = '["*"]', page = 1, length = 250, filters = '[]' } = req.query;
...
async function getForJournalForReddys(doctype, filters) {
    
    // This API call fetches just the count of the filtered records
    let url = `https://reddyspharma.accu360.cloud/api/resource/${doctype}?fields=["name"]&filters=${filters}&limit_page_length=*`;
    console.log('Fetching total records URL:', url);
...
app.get('/get-reddys-stock-entry', async (req, res) => {
    const { doctype = 'Stock Entry', fields = '["*"]', page = 1, length = 250, filters = '[]' } = req.query;
